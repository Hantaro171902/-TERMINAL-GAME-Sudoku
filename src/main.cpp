...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (atoi(argv[i]) > 0) gen = new Generator(atoi(argv[i]));
            ...
            gen = new Generator(getRandomXMLPuzzle(argv[i+1]).c_str()); ++i; 
        }
        else if (strcmp(argv[i], "--sdm") == 0 && !gen && i+1 < argc) { 
            gen = new Generator(getRandomSdmPuzzle(argv[i+1]).c_str()); ++i; 
        }
        }
